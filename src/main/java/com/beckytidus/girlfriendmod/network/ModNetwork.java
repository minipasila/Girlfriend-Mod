...
public class ModNetwork {
    ...
    public record ConfigPayload(String apiKey, String model, int tokens) implements CustomPayload {
    ...
    private static ConfigPayload readConfig(PacketByteBuf buf) {
        return new ConfigPayload(buf.readString(), buf.readString(), buf.readInt());
    }
    ...
    public static void register() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ClearMemoryPayload.ID, (payload, context) -> {
            context.server().execute(() -> {
                ...
                // 1.21.9 API Change: Entity#getWorld() -> Entity#getEntityWorld()
    ...
    public static void sendConfigUpdatePacket(ModConfig config) {
        ClientPlayNetworking.send(new ConfigPayload(config.apiKey, config.modelName, config.maxHistoryTokens));
    }
